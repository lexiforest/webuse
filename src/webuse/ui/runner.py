...
class Runner:
    def __init__(
        ...
        *,
        ...
        self.concurrency = max(
            1, concurrency or int(os.environ.get("WEBUSE_WORKER_CONCURRENCY", "1"))
        )
        self.poll_seconds = max(
            0.25,
            (poll_ms or int(os.environ.get("WEBUSE_WORKER_POLL_MS", "1000"))) / 1000,
        )
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._loop, name="webuse-ui-runner", daemon=True
        )
        self._thread.start()
    ...
    def _run_job(self, job: dict[str, Any]) -> None:
        ...
        if not project:
            self.store.append_log(
                job["id"], "stderr", f"project not found: {job['projectId']}"
            )
        ...
        crawl_target = (
            ...
            else (stored_config_path or job_dir / "spider.yaml")
        )
        output_path = job_dir / "items.jsonl"
        if project.get("type") != "git" and stored_config_path is None:
            crawl_target.write_text(
                yaml.safe_dump(_normalize_config(project), sort_keys=False),
                encoding="utf-8",
            )
        ...
        try:
            stdout_thread = threading.Thread(
                ...
                args=(
                    child.stdout,
                    lambda line: self.store.append_log(job["id"], "stdout", line),
                ),
                daemon=True,
            )
            stderr_thread = threading.Thread(
                ...
                args=(
                    child.stderr,
                    lambda line: self.store.append_log(job["id"], "stderr", line),
                ),
                daemon=True,
            )
            stdout_thread.start()
        ...
        if code < 0:
            self.store.append_log(
                job["id"], "stderr", f"crawl terminated by signal {-code}"
            )
            self.store.finish_job(
                job["id"],
                "cancelled",
                {"reason": "signal", "signal": -code, "outputPath": str(output_path)},
            )
            return
        if code != 0:
            ...
            self.store.finish_job(
                job["id"],
                "failed",
                {"reason": "exit_code", "code": code, "outputPath": str(output_path)},
            )
            return

        item_count = (
            _import_jsonl(self.store, job["id"], output_path)
        ...
        self.store.append_log(
            job["id"],
            "stdout",
            f"imported {item_count} item{'s' if item_count != 1 else ''}",
        )
        self.store.finish_job(
            job["id"],
            "succeeded",
            {"outputPath": str(output_path), "itemCount": item_count},
        )
